#pragma once

#include <gtest/gtest.h>

#include "../PowFunction/functions.cpp"

TEST(PowFunctionTest, MyPowTest2_2)
{
    ASSERT_DOUBLE_EQ(4, my_pow(2, 2));
}

TEST(PowFunctionTest, MyPowTest3_3)
{
    ASSERT_DOUBLE_EQ(27, my_pow(3, 3));
}

TEST(PowFunctionTest, MyPowTest0_3)
{
    ASSERT_DOUBLE_EQ(0, my_pow(0, 3));
}

TEST(PowFunctionTest, MyPowTest3_0)
{
    ASSERT_DOUBLE_EQ(1, my_pow(3, 0));
}

TEST(PowFunctionTest, MyPowTest4_min2)
{
    ASSERT_DOUBLE_EQ(0.0625, my_pow(4, -2));
}
